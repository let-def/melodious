...
SHARED_FILES := utils_generic.ml utils.ml model.ml 
## Source files for the server part

SERVER_FILES := ${SHARED_FILES} sql_utils.ml \
	repository.ml database.ml auth.ml ${wildcard *.eliom}

## Source files for the client part

CLIENT_FILES := ${SHARED_FILES} utils_client.ml dom_media.ml table_view.ml \
	audio.ml melodious.ml proto.ml ${wildcard *.eliom} 
...
reload: install
	echo "reload" >> /tmp/ocsigen_command
